...
from pretix.base.models import Event, Invoice, Order, Organizer
...
class LogMiddleware:
    ...
    def __call__(self, request):
        ...
        if (
            request.path.startswith(get_script_prefix() + "api")
            and request.user.is_authenticated
    ...
    def _log_control_request(self, request):
        ...
        if (
            url.url_name in ("event.orders.export.do", "event.shredder.export")
    ...
    def _log_api_request(self, request, response):
        ...
        if codes:
            for order in Order.objects.filter(
                event__slug=url.kwargs["event"], code__in=codes
            ):
                order.log_action(
                    "pretix_log_read_access.order.read",
                    user=request.user,
                    auth=request.auth,
                )
        ...
        if "exporters-run" in url.url_name and request.method == "POST":
            ...
            object.log_action(
                "pretix_log_read_access.export",
                data={"exporter": url.kwargs.get("pk"), "api": True},
                user=request.user,
                auth=request.auth,
            )
    ...
    def _log_order(self, order, user=None, auth=None):
        ...
        if user:
            if (
                not order.all_logentries()
                .filter(
                    action_type="pretix_log_read_access.order.read",
                    user=user,
                ...
                order.log_action(
                    "pretix_log_read_access.order.read", user=user, auth=auth
                )
        else:
    ...
    def _log_export(self, object, data, user=None, auth=None):
        ...
        object.log_action(
            "pretix_log_read_access.export", data=data, user=user, auth=auth
        )
    ...
    def _check_order_list(self, request):
        for k, v in request.GET.items():
            if (
                v
                and (k == "answer" or "question_" in k)
                and not request.user.has_active_staff_session(
                    request.session.session_key
                )
            ):
                raise PermissionDenied(
                    "This type of search is only allowed for system administrators."
                )
